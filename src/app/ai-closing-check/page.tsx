...
export default function AIClosingCheckPage() {
  ...
  const _handleStartCheck = async () => {
    setRows([
      ...
      { id: 3, category: '필수', description: '매출채권 연령 분석', status: 'pending' },
      { id: 4, category: '외부감사시 필수', description: '재고자산 실사', status: 'pending' },
      { id: 5, category: '정합성', description: '감가상각 누계 확인', status: 'pending' },
    ]);
    ...
    <div className="p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            <h2 className="text-xl font-bold mb-2 text-[#1E1E1E]">AI 결산점검</h2>
        ...
        <table className="w-full border border-[#D9D9D9] text-sm">
          <thead>
            <tr>
              ...
              <th className="bg-[#F5F5F5] p-3 border border-[#D9D9D9]">점검항목</th>
              <th className="bg-[#F5F5F5] p-3 border border-[#D9D9D9]">내용</th>
              <th className="bg-[#F5F5F5] p-3 border border-[#D9D9D9]">처리현황</th>
              <th className="bg-[#F5F5F5] p-3 border border-[#D9D9D9]">점검/수정</th>
            </tr>
          </thead>
          <tbody>
            {rows.map(r => (
              <tr key={r.id}>
                <td className="p-3 border border-[#D9D9D9] text-red-500">{r.category}</td>
                <td className="p-3 border border-[#D9D9D9]">{r.description}</td>
                <td className="p-3 border border-[#D9D9D9]">입력하기</td>
                <td className="p-3 border border-[#D9D9D9]">{renderStatus(r.status)}</td>
