                  ...
                  <tbody>
                    {endingInventoryVoucherData && endingInventoryVoucherData.transactions?.length > 0 ? (
                      <>
                        {(() => {
                          // 차변과 대변 거래 분리
                          const debitTransactions = endingInventoryVoucherData.transactions.filter(t => t.debitCredit);
                          const creditTransactions = endingInventoryVoucherData.transactions.filter(t => !t.debitCredit);
                            ...
                            return (
                              <tr key={`row-${rowIndex}`} className="h-8">
                                <td className="p-2 border border-[#D9D9D9] text-center h-8">
                                  {rowIndex === 0 ? formatDate(endingInventoryVoucherData.voucher.date) : ''}
                                </td>
                                {/* 차변 칸 */}
                                {debitTx ? (
                                  <>
