...
import { Form27Data, Form27InputData, } from '@/components/taxDocument/template/Form27/type';
...
export default function Form27({
  ...
  const displayBizType = (() => {
    if (typeof bizType !== 'string') return bizType ?? '';
    const trimmed = bizType.trim();
    if (!trimmed.startsWith('[') || !trimmed.endsWith(']')) return bizType;
    try {
  ...
  const displayBizItem = (() => {
    if (typeof bizItem !== 'string') return bizItem ?? '';
    const trimmed = bizItem.trim();
    if (!trimmed.startsWith('[') || !trimmed.endsWith(']')) return bizItem;
    try {
      const parsed = JSON.parse(trimmed);
      if (Array.isArray(parsed)) {
        return parsed.filter(item => typeof item === 'string').join(', ');
  ...
  return (
    <PageSlot slotWidth={880} slotHeight={1243.85}>
      <div className="form27">
        ...
        >
          「부가가치세법 시행령」 제90조제3항의 표 제7호, 제91조제2항의 표
          제10호 및 제107조제3항에 따라 건물 등 감가상각자산 취득명세서를
          제출합니다.
        </p>
        <p
          className="s5"
